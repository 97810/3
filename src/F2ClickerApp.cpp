...
bool F2ClickerApp::CreateMainWindow(int show_command) {
    main_window_ = CreateWindowExW(0, kWindowClassName, L"热键点击器",
        ...
        CW_USEDEFAULT, CW_USEDEFAULT, 570, 365, nullptr, nullptr, instance_, this);
...
LRESULT CALLBACK F2ClickerApp::StaticWindowProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
    ...
    if (message == WM_NCCREATE) {
        ...
        if (app) {
            app->main_window_ = hwnd;  // 关键修复：WM_CREATE 前先保存主窗口句柄
...
void F2ClickerApp::CreateControls() {
    ...
    label_info_ = CreateLabel(main_window_, L"状态：", 24, 178, 510, 52);
    CreateLabel(main_window_, L"点击键：绑定 / 开始计时 / 结束并点击；快速键：0.5 秒后点击", 24, 238, 510, 24);
    CreateButton(main_window_, L"重置绑定", kButtonReset, 356, 282, 90, 30);
    CreateButton(main_window_, L"退出", kButtonExit, 456, 282, 90, 30);
    PopulateKeyCombos();
    UpdateHotkeyLabels();
}
...
void F2ClickerApp::OnClickHotkey() {
    ...
    if (!timer_overlay_.IsVisible()) {
        ...
        UpdateStatus(L"已点击，计时开始。再次按点击键将结束计时。 ");
    } else {
        timer_overlay_.Hide();
        audio_player_.PlayEnd();
        UpdateStatus(L"已点击，计时结束。 ");
    }
}
...
void F2ClickerApp::UpdateBindingLabels() {
    ...
    const POINT point = click_controller_.TargetClientPoint();
    std::wostringstream coord;
    coord << L"绑定坐标（窗口内）：" << point.x << L", " << point.y;
    SetWindowTextW(label_coord_, coord.str().c_str());
    SetWindowTextW(label_handle_, FormatWindowHandle(click_controller_.TargetWindow()).c_str());
}
